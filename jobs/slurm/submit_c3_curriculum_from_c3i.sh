...
BASE_LABEL="${BASE_LABEL:-c3i_sat010_e699}"

if [ ! -f "$BASE_CKPT" ]; then
  echo "[submit-c3-curriculum] Missing base checkpoint: $BASE_CKPT" >&2
  echo "[submit-c3-curriculum] Set BASE_CKPT=... to override or rsync c3i sat010 to the cluster first." >&2
  exit 1
fi

STAMP="${STAMP:-$(date +%Y%m%d_%H%M%S)}"
SUBMISSIONS="logs/remote_runs/c3_curriculum_submissions_${STAMP}.tsv"
...
submit_stage() {
  ...
  printf '[submit-c3-curriculum] stage=%s run_name=%s\n' "$stage" "$run_name"
  output="$(
    sbatch \
      --job-name "$job_name" \
...
printf '[submit-c3-curriculum] base_ckpt=%s\n' "$BASE_CKPT"
submit_stage "roll-c3k-soft" "c3k_720_soft_polish"
submit_stage "roll-c3l-xy" "c3l_720_xy_guard"
submit_stage "roll-c3m-sat" "c3m_720_smooth_sat_guard"
printf '[submit-c3-curriculum] wrote=%s\n' "$SUBMISSIONS"
