from mjlab.envs.mdp import base_ang_vel, base_lin_vel

from .observations import (
    base_quat_wxyz,
    depth_error_from_ref,
    last_body_wrench_action,
    phi_total_norm,
    xy_error_w,
)
from .metrics import (
    PitchAbsPeakRad,
    XyDriftPeakM,
    body_wrench_action_l2,
    body_wrench_action_rate_l2 as body_wrench_action_rate_l2_metric,
    body_wrench_saturation_fraction,
    depth_abs_error_m,
    hydro_wrench_norm,
    nonroll_body_wrench_action_rate_l2 as nonroll_body_wrench_action_rate_l2_metric,
    phi_total_rad,
    pitch_abs_rad,
    PostTargetRollThroughTorqueMean,
    post_target_roll_through_torque,
    roll_progress_ratio,
    root_ang_speed_rad_s,
    settle_counter_s,
    target_reached,
    water_current_speed_m_s,
    x_drift_m,
    xy_drift_m,
    y_drift_m,
    yaw_abs_error_rad,
)
from .rewards import (
    body_wrench_action_effort,
    body_wrench_action_rate_l2,
    depth_hold_penalty,
    nonroll_body_wrench_action_rate_l2,
    pitch_penalty,
    post_target_roll_through_torque_penalty,
    roll_progress,
    terminal_failure_reward,
    terminal_success_reward,
    thruster_saturation_cost,
    xy_drift_penalty,
    yaw_hold_penalty,
)
from .terminations import (
    excess_depth_error,
    excess_pitch,
    excess_xy_drift,
    roll_task_success,
)

__all__ = [
    "base_ang_vel",
    "base_lin_vel",
    "base_quat_wxyz",
    "body_wrench_action_l2",
    "body_wrench_action_effort",
    "body_wrench_action_rate_l2",
    "body_wrench_action_rate_l2_metric",
    "body_wrench_saturation_fraction",
    "depth_abs_error_m",
    "depth_error_from_ref",
    "depth_hold_penalty",
    "excess_depth_error",
    "excess_pitch",
    "excess_xy_drift",
    "hydro_wrench_norm",
    "last_body_wrench_action",
    "nonroll_body_wrench_action_rate_l2_metric",
    "phi_total_rad",
    "phi_total_norm",
    "PitchAbsPeakRad",
    "nonroll_body_wrench_action_rate_l2",
    "pitch_penalty",
    "pitch_abs_rad",
    "PostTargetRollThroughTorqueMean",
    "post_target_roll_through_torque_penalty",
    "post_target_roll_through_torque",
    "roll_progress",
    "roll_progress_ratio",
    "roll_task_success",
    "root_ang_speed_rad_s",
    "settle_counter_s",
    "target_reached",
    "terminal_failure_reward",
    "terminal_success_reward",
    "thruster_saturation_cost",
    "water_current_speed_m_s",
    "x_drift_m",
    "XyDriftPeakM",
    "xy_drift_m",
    "xy_drift_penalty",
    "xy_error_w",
    "y_drift_m",
    "yaw_abs_error_rad",
    "yaw_hold_penalty",
]
